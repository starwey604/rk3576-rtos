...
rt_err_t rt_usbh_detach_instance(uinst_t device)
{
    ...
    if (device->cfg_desc) {
        for (i = 0; i < device->cfg_desc->bNumInterfaces; i++)
        {
...
#if defined(RT_USBH_MSTORAGE)
            if (device->intf[i]->drv->class_code == USB_CLASS_MASS_STORAGE)
            {
                ustor_t stor = (ustor_t)device->intf[i]->user_data;

                if (stor->use_cmd_unmount && stor->mount_status)
                {
                    timeout = USB_TIMEOUT_LONG;
                    rt_completion_init(&device->intf[i]->intf_completion);
                }
            }
#endif
            ...
            if (timeout > 0)
                rt_completion_wait(&device->intf[i]->intf_completion, timeout);
...
int rt_usb_hcd_pipe_xfer(uhcd_t hcd, upipe_t pipe, void* buffer, int nbytes, int timeout)
{
    ...
    rt_size_t send_size;
    rt_size_t actual_size;
    remain_size = nbytes;
    rt_uint32_t maxpacket;
    rt_uint8_t * pbuffer = (rt_uint8_t *)buffer;

    if (hcd == RT_NULL ||  pipe == RT_NULL)
    {
        rt_kprintf("the hcd or pipe is not available\n");
        return 0;
    }

#if defined(RT_USBH_PIPE_XFER_MAX_SIZE)
    maxpacket = RT_USBH_PIPE_XFER_MAX_SIZE;
#else
    maxpacket = pipe->ep.wMaxPacketSize;
#endif

    do
    {
        RT_DEBUG_LOG(RT_DEBUG_USB,("pipe transform remain size,: %d\n", remain_size));
        send_size = (remain_size > maxpacket) ? maxpacket : remain_size;
        actual_size = hcd->ops->pipe_xfer(pipe, USBH_PID_DATA, pbuffer, send_size, timeout);
        if(actual_size == send_size)
        {
        ...
        {
            actual_size = (actual_size >= 0) ? (nbytes - remain_size + actual_size) : 0;
            return actual_size;
        }
    }while(remain_size > 0);
    return nbytes;
