    ...
    ustorage_cbw_t cmd, rt_uint8_t* buffer, int timeout)
{
    ...
    if (!stor->udisk_connected && stor->mount_status)
    {
        rt_kprintf("bulk xfer error: udisk disconnect\n");
        return -RT_EIO;
    }
...
err:
    if (stor->udisk_connected && intf->device->hcd->ops->force_disconnect)
        intf->device->hcd->ops->force_disconnect(intf->device->hcd->parent.user_data);
...
static rt_err_t rt_usbh_storage_disable(void* arg)
{
    ...
    if(stor != RT_NULL && !stor->use_cmd_unmount) rt_free(stor);
    return RT_EOK;
}
