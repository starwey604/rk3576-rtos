...
static void lvgl_task(void *arg)
{
    ...
    rt_kprintf("sram total=%d, used=%d, maxm=%d\n", total, used, maxm);

    lv_init();

#ifdef RT_USING_LVGL9
    lv_tick_set_cb(rt_tick_get);
    lv_delay_set_cb(rt_thread_mdelay);
#endif
    ...
    lv_demo_benchmark();
#elif defined(RT_LV_DEMO_WIDGETS)
    lv_demo_widgets();
