...
#ifdef RT_USING_IRQ_PREEMPT
/* ISB ensures any pending IRQ is taken immediately after enabling,
 * minimizing preemption latency. */
#define _irq_enable()   __asm__ volatile ("msr daifclr, #2\nisb" ::: "memory")
#define _irq_disable()  __asm__ volatile ("msr daifset, #2\nisb" ::: "memory")
#endif
...
void rt_hw_trap_irq(void)
{
    ...
    if (isr_func)
    {
         ...
         * GIC running priority (set by IAR) already masks same/lower priority,
         * and naturally bounds nesting depth to the number of priority levels. */
        _irq_enable();
