...
def check_xip_addr(setting_file) :
    if ".ini" in setting_file:
        search_list = ['File=../../Image/rtt0.img', 'File=../../Image/rtt0_b.img', 'File=../../Image/recovery.img']
    else:
        parameter_file = setting_file.replace("package-file", "parameter.txt")
        search_list = ['cpu0_os_a', 'recovery']
        img_list = ['rtt0.img', 'recovery.img']
    ...
    for index, search_string in enumerate(search_list) :
        ...
        if os.path.exists(elf_file) :
            ...
            if vectors_address < xip_base or vectors_address >= xip_end :
                print("vectors address is not in the XIP range, check ignore")
                if elf_file == "rtt0.elf" or elf_file == "recovery.elf" :
...
def package_fw(setting_file):
    ...
    # rtt和recovery打包
    search_list = ['../../Image/rtt0.img', '../../Image/rtt0_b.img', '../../Image/recovery.img']
