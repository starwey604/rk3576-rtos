...
usage() {
    echo "Usage:"
    echo "    $0 [partition_setting] [rtt0|rtt1|recovery|soundbar|partybox|gamebar|audio_pipe|rkstudio_test]"
    echo
    echo "    Build all firmware"
    echo "        $0 board/adsp_demo/setting.ini            : output Firmware.img, default framework"
    echo "        $0 board/adsp_demo/setting.ini audio_pipe : output Firmware.img, audio_pipe framework"
    echo "            * RK_AUTOMOTIVE_DSP_DEMO_RK2118M_V10_2024-01-25 or RK_EVB2_AUTOMOTIVE_DSP_RK2118M_V20_20240415"
    echo
    echo "        $0 board/evb/package-file                 : output update.img"
    echo "            * RK_EVB1_RK2118G_V10_20240219 (spi nor flash)"
    echo
    echo "        $0 board/evb_nand/package-file            : output update.img"
    echo "            * RK_EVB1_RK2118G_V10_20240219 (spi nand flash)"
    echo
    echo "        $0 board/evb1_v20/package-file            : output update.img"
    echo "            * RK_EVB1_RK2118G_V20_20240424 or RK_EVB1_RK2118G_V21_20241010 (spi nand flash)"
    echo
    echo "        $0 board/evb3_v10/package-file            : output update.img"
    echo "            * RK_EVB3_RK2118G_V10_20240715 (spi nand flash)"
    echo
    ...
    echo "        $0 board/evb/package-file soundbar        : build soundbar demo"
    echo "            * components/hifi4/rtt/dsp_fw/evb_soundbar_demo/"
...
if [[ $setting_file == *".ini"* ]]; then
    if grep -q "rk2118_idb_ddr.img" "$setting_file"; then
    ...
    if [ "$2" == "audio_pipe" ]; then
        DEFCONFIG_PATH0="board/${BOARD_NAME}/defconfig_audio_pipe"
    fi
else
    echo "RTT_BUILD_XIP=N"
    export RTT_BUILD_XIP=N
...
if [ "$#" -eq 2 ]; then
    if [[ "$2" == "rtt0" || "$2" == "rtt0_b" || "$2" == "rtt1" || "$2" == "recovery" ]]; then
        ...
        exit 0
    elif [[ "$2" == "soundbar" || "$2" == "partybox" || "$2" == "gamebar" || "$2" == "audio_pipe" || "$2" == "rkstudio_test" ]]; then
        echo ""
    else
        usage
        exit 1
