...
usage() {
    echo "usage: ./mkimage.sh [partition_setting] [soundbar|partybox|gamebar|audio_pipe|rkstudio_test]"
}
...
replace_demo() {
  if [[ "$1" != "soundbar" && "$1" != "partybox" && "$1" != "gamebar" && "$1" != "audio_pipe" && "$1" != "rkstudio_test" ]]; then
    echo "Invalid argument. Please pass 'soundbar', 'partybox', 'gamebar' or 'audio_pipe'."
    usage
    return 1
  fi
...
replace_dsp_bin() {
    DSP_BIN_FILE="../../../../../components/hifi4/rtt/dsp_fw/vehicle_audio_pipe"
    RKSTUDIO_BIN_FILE="../../../../../components/hifi4/rtt/rkstudio_bin/vehicle_audio_pipe/vehicle_audio_pipe.bin"
    # Replace dsp bin path with $DSP_BIN_FILE in setting.ini
    sed -i "s|File=.*dsp0.bin|File=$DSP_BIN_FILE/dsp0.bin|g" "$1"
    sed -i "s|File=.*dsp1.bin|File=$DSP_BIN_FILE/dsp1.bin|g" "$1"
    sed -i "s|File=.*dsp2.bin|File=$DSP_BIN_FILE/dsp2.bin|g" "$1"
    # Replace rkstudio bin path with $RKSTUDIO_BIN_FILE in setting.ini
    sed -i "s|File=.*\/rkstudio\.bin|File=$RKSTUDIO_BIN_FILE|g" "$1"
}
