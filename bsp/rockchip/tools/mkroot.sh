...
make_root_fs() {
    ...
    root_size_thr_kb=2048
    root_inc_size_thr_kb=2048

    # 计算 resource/* 文件的总大小
    total_size=$(du -c $resource_dir/* | grep total$ | awk '{print $1}')
    echo "resource total_szie_kb=$total_size"

    if [ "$root_size_byte" -le $((root_size_thr_kb * 1024)) ]; then
        total_size=$(( total_size + 256 )) # increase 1 block
        echo "increased resource total_szie_kb=$total_size"
        if [ "$total_size" -gt "$root_size_thr_kb" ]; then
            echo "Error: resource files is $total_size (kb) greater than root partition size $root_size_byte byte"
            echo "Please enlarge root partition size"
            exit 1
        fi
        ...
        total_size_10=$(( total_size / 10 ))
        if [ "$total_size_10" -le "$root_inc_size_thr_kb" ]; then
            total_size=$(( total_size + root_inc_size_thr_kb ))
        else
            total_size=$(( total_size + total_size_10 ))
        fi
        echo "increased resource total_szie_kb=$total_size"

        fat_size_byte=$(( total_size * 1024 ))
        fat_size_mb=$(( fat_size_byte / 1024 / 1024 ))
