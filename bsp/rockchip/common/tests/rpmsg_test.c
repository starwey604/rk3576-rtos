...
#define LINK_ID_0   ((uint32_t)0)
#define LINK_ID_1   ((uint32_t)1)
#define LINK_ID_2   ((uint32_t)2)
#define LINK_ID_3   ((uint32_t)3)
#define LINK_ID_4   ((uint32_t)4)
...
#define RPMSG_RTT_REMOTE_TEST4_EPT_ID 0x3005U
#define RPMSG_RTT_REMOTE_TEST_EPT4_NAME "rpmsg-mcu0-test"
#endif

#define RPMSG_RTT_TEST_MSG "Rockchip rpmsg linux test!"
#define RPMSG_RTT_TEST_MSG_ANDROID "Rockchip rpmsg android test!"
...
#define RPMSG_ANDROID_MEM_SIZE (2UL * RL_VRING_OVERHEAD)
...
static void rpmsg_share_android_mem_check(void)
{
    if ((RPMSG_ANDROID_MEM_BASE + RPMSG_ANDROID_MEM_SIZE) > RPMSG_ANDROID_MEM_END)
    {
        rt_kprintf("share memory size error!\n");
        while (1)
        {
...
static void rpmsg_linux_test(void)
{
    ...
    set_os_sel_irq(0);
    info->instance = rpmsg_lite_remote_init((void *)RPMSG_LINUX_MEM_BASE, LINK_ID_0, RL_NO_FLAGS);
    rpmsg_lite_wait_for_link_up(info->instance, 10U);
    printf("rpmsg remote:linux  link up! link_id-0x%" PRIx32 "\n", info->instance->link_id);
    rpmsg_ns_bind(info->instance, rpmsg_ns_cb, &ns_cb_data);
    info->cb_sta = 0;
    info->ept = rpmsg_lite_create_ept(info->instance, RPMSG_RTT_REMOTE_TEST3_EPT_ID, remote_ept_cb, info);
    ept_flags = RL_NS_CREATE;
    rpmsg_ns_announce(info->instance, info->ept, RPMSG_RTT_REMOTE_TEST_EPT3_NAME, ept_flags);
    block = (struct rpmsg_block_t *)info->private;
    while (1)
    {
        if (info->cb_sta == 1)
        {
            info->cb_sta = 0;
            printf("buffer len =%d\n", strlen(block->buffer));
            rt_memset(rx_msg, 0, RL_BUFFER_PAYLOAD_SIZE);
            rpmsg_lite_send(info->instance, info->ept, info->m_ept_id, block->buffer, strlen(block->buffer), RL_BLOCK);
        }
        else
        {
...
static int32_t remote_ept_cb_second(void *payload, uint32_t payload_len, uint32_t src, void *priv)
{
    int ret;
    struct rpmsg_info_t *info = (struct rpmsg_info_t *)priv;
    struct rpmsg_block_t *block = (struct rpmsg_block_t *)info->private;
...
static void rpmsg_android_test(void)
{
    struct rpmsg_info_t *info;
    struct rpmsg_block_t *block;
    uint32_t ept_flags;
    ...
    rpmsg_share_android_mem_check();
    printf("rpmsg android remote: shmem_base-0x%" PRIx32 " shmem_end-%" PRIx32 "\n", RPMSG_ANDROID_MEM_BASE, RPMSG_ANDROID_MEM_END);
    ...
    set_os_sel_irq(1);
    info->instance = rpmsg_lite_remote_init((void *)RPMSG_ANDROID_MEM_BASE, LINK_ID_1, RL_NO_FLAGS);
    rpmsg_lite_wait_for_link_up(info->instance, 10U);
    printf("rpmsg remote: link up! link_id-0x%" PRIx32 "\n", info->instance->link_id);
    rpmsg_ns_bind(info->instance, rpmsg_ns_cb, &ns_cb_data);
    info->cb_sta = 0;
    info->ept = rpmsg_lite_create_ept(info->instance, RPMSG_RTT_REMOTE_TEST4_EPT_ID, remote_ept_cb_second, info);
    ept_flags = RL_NS_CREATE;
    rpmsg_ns_announce(info->instance, info->ept, RPMSG_RTT_REMOTE_TEST_EPT4_NAME, ept_flags);
    block = (struct rpmsg_block_t *)info->private;
    while (1)
    {
        if (info->cb_sta == 1)
...
void rpmsg_remote(void)
{
    rt_thread_t rtt_linux;
    rt_thread_t rtt_android;
    rt_kprintf("rpmsg_remote start \n");

    rtt_android = rt_thread_create("rpmsg_android_remote", rpmsg_android_test, RT_NULL, 1024 * 2, 4, 10);
    if (rtt_android != RT_NULL)
    {
        rt_thread_startup(rtt_android);
    }

    rtt_linux = rt_thread_create("rpmsg_linux_remote", rpmsg_linux_test, RT_NULL, 1024 * 2, 3, 10);
    if (rtt_linux != RT_NULL)
    {
        rt_thread_startup(rtt_linux);
    }


    rt_kprintf("rpmsg_remote end\n");
}
INIT_APP_EXPORT(rpmsg_remote);
#endif
