...
void dma_memcpy(int *dst_mem, int *src_mem, int size)
{
    ...
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)src_mem, size);
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)dst_mem, size);
    ...
    m2m_xfer->dst_addr = (uintptr_t)dst_mem;
    m2m_xfer->src_addr = (uintptr_t)src_mem;
    m2m_xfer->len = size;
...
static int plc_simple_test(struct rockchip_rt_dsmc_host *dsmc_host, uint32_t cs)
{
    ...
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)src, size);
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)map->phys, size);

    timeout = 10000;
    if (!ops->copy_to(dsmc_host, cs, 0, (uintptr_t)src, 0x0, size))
    {
    ...
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)dst, size);
    HAL_DCACHE_CleanInvalidateByRange((uintptr_t)map->phys, size);

    timeout = 10000;
    if (!ops->copy_from(dsmc_host, cs, 0x0, 0, (uintptr_t)dst, size))
    {
