...
static struct MBOX_REG *rl_pMBox[MBOX_CNT] = { HPMCU_MBOX0, HPMCU_MBOX1, HPMCU_MBOX2, HPMCU_MBOX3 };
...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    ...
    if (isr_counter < 2 * RL_MAX_INSTANCE_NUM)
    {
        ...
        if (register_count % 2 == 0)
        {
            ...
            ret = HAL_MBOX_RegisterClient(rl_pMBox[cpu_id], MBOX_CH_0, &mbox_clr[cpu_id]);
            if (ret)
            {
            ...
            ret = HAL_MBOX_RegisterClient(rl_pMBox[RL_GET_M_CPU_ID(vector_id)], MBOX_CH_0, &mbox_clr[RL_GET_M_CPU_ID(vector_id)]);
            if (ret)
            {
                rt_kprintf("mbox remote client register failed, ret=%d\n", ret);
            }

            rt_hw_interrupt_install(mbox_clr[cpu_id].irq, rpmsg_mbox_isr, rl_pMBox[cpu_id], "rpmsg-lite");
        }
...
int32_t platform_interrupt_enable(uint32_t vector_id)
{
    ...
    if (disable_counter < 2 * RL_MAX_INSTANCE_NUM)
    {
        ...
        if (cpu_id == RL_GET_M_CPU_ID(vector_id))
            rt_hw_interrupt_umask(mbox_clm[cpu_id].irq);
        else
            rt_hw_interrupt_umask(mbox_clr[cpu_id].irq);
#endif /* RL_PLATFORM_USING_MBOX */
    }
...
#ifdef RL_PLATFORM_USING_MBOX
        if (cpu_id == RL_GET_M_CPU_ID(vector_id))
            rt_hw_interrupt_mask(mbox_clm[cpu_id].irq);
        else
            rt_hw_interrupt_mask(mbox_clr[cpu_id].irq);
#endif /* RL_PLATFORM_USING_MBOX */
    }
    disable_counter++;
    platform_global_isr_enable();
