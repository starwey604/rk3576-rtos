...
static rt_err_t DSMC_HOST_RemapMemory(struct rockchip_rt_dsmc_host *dsmc_host,
                                      unsigned long mem_attr)
{
    ...
    {
        cfg = &dsmcHostDev->ChipSelCfg[cs];
        if (cfg->deviceType == DEV_UNKNOWN)
        {
            continue;
        }

        if (cfg->deviceType == DEV_XCCELA_PSRAM)
        {
            regionMap = &dsmcHostDev->ChipSelMap[cs].regionMap[0];
            ret = rt_hw_mmu_map(regionMap->phys, regionMap->size, mem_attr);
            rt_hw_mmu_tlb_invalidate();
        }
        else if (cfg->deviceType == DSMC_LB_DEVICE)
        {
            for (rgn = 0; rgn < DSMC_LB_MAX_RGN; rgn++)
            {
                if (!cfg->slvRgn[rgn].status)
                    continue;
                regionMap = &dsmcHostDev->ChipSelMap[cs].regionMap[rgn];
                if (cfg->slvRgn[rgn].attribute != RGNX_ATTR_REG)
                {
                    ret = rt_hw_mmu_map(regionMap->phys,
                                        regionMap->size,
                                        mem_attr);
                }
                rt_hw_mmu_tlb_invalidate();
...
static rt_err_t rockchip_dsmc_host_dma_configure(struct rockchip_rt_dsmc_host *dsmc_host)
{
    ...
    {
        dsmc_host->dma = rt_dma_get((uintptr_t)(dsmc_host->host->dmaReq[0].dmac));
