...
struct rockchip_usbotgh
{
    ...
    struct rt_completion urb_completion[OTG_HCD_MAX_CHANNEL];
...
static void rt_usbotgh_force_disconnect(void *data);
...
{                                                               \
    ...
    .force_disconnect = rt_usbotgh_force_disconnect,            \
};                                                              \
...
{
    usbotgh_t usbotgh = (usbotgh_t)hhcd->pData;
    rt_completion_done(&usbotgh->urb_completion[chnum]);
}
...
static int rt_usbotgh_pipe_xfer(upipe_t pipe, rt_uint8_t token, void *buffer, int nbytes, int timeout)
{
    ...
    rt_uint8_t chnum = pipe->pipe_index;
    ...
    while (1)
    {
        ...
        rt_completion_init(&usbotgh->urb_completion[chnum]);
        HAL_HCD_HCSubmitRequest(&usbotgh->instance,
                                ...
                                nbytes,
                                0);
        rt_completion_wait(&usbotgh->urb_completion[chnum], timeout);

        hc_state = HAL_HCD_HCGetState(&usbotgh->instance, chnum);
        urb_state = HAL_HCD_HCGetURBState(&usbotgh->instance, chnum);
        ...
        if (hc_state == HC_NAK)
        {
            ...
            HAL_HCD_HCHalt(&usbotgh->instance, chnum);
        ...
        else if (urb_state == URB_ERROR)
        {
            ...
            {
                HAL_HCD_Disconnect_Callback(&usbotgh->instance);
        ...
        else if ((urb_state == URB_IDLE && hc_state == HC_IDLE && pipe->inst->intf[0] != RT_NULL &&
                  pipe->inst->intf[0]->intf_desc->bInterfaceClass == USB_CLASS_MASS_STORAGE) ||
                 (urb_state == URB_IDLE && hc_state == HC_IDLE && pipe->inst->intf[0] == RT_NULL) ||
        ...
        else if (urb_state != URB_NOTREADY && urb_state != URB_NYET)
        {
            ...
            {
                return HAL_HCD_HCGetXferCount(&usbotgh->instance, chnum);
            }
            return nbytes;
        }
        return -1;
...
static void rt_usbotgh_force_disconnect(void *data)
{
    ...
    if (data != RT_NULL)
    {
        usbotgh = (usbotgh_t)data;
        hcd = (uhcd_t)usbotgh->uhcd;

        if (usbotgh->connect_status)
        {
            usbotgh->connect_status = RT_FALSE;
            rt_kprintf("USB force disconnect\n");
            rt_usbh_root_hub_disconnect_handler(hcd, OTG_HCD_PORT);
        }
    }
}
