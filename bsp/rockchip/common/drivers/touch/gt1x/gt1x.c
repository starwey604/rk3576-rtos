...
static rt_err_t gt1x_control(struct rt_touch_device *device, int cmd, void *data)
{
    ...
    else if (cmd == RT_TOUCH_CTRL_GET_INFO)
    {
        ...
        {
            info->range_x = gt1x_abs_x_max;
            info->range_y = gt1x_abs_y_max;
        }
        else
        {
...
int gt1x_reset_guitar(void)
{
    ...
    rt_thread_mdelay(50);
#ifdef TOUCH_IRQ_PIN_MODE
    rt_pin_mode(TOUCH_IRQ_PIN, TOUCH_IRQ_PIN_MODE);
#else   //default
...
#ifdef TOUCH_IRQ_PIN_MODE
    cfg.irq_pin.mode = TOUCH_IRQ_PIN_MODE;
#else   //default
