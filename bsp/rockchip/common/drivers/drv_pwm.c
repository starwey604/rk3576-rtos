...
struct rockchip_pwm
{
    ...
    rt_bool_t is_counter_enable;

    /* irq handler */
    rt_isr_handler_t irq_handler;

    rt_event_t freq_event;
};
...
static void rockchip_pwm_irq(struct rockchip_pwm *pwm)
{
    ...
    for (i = 0; i < pPWM->channelNum; i++)
    {
        ...
        /*if (pPWM->pChHandle[i].freqMeterResValid)
            rt_event_send(pwm->freq_event, 0x1);*/
    }
#else
...
static rt_bool_t rockchip_pwm_support_pclk_dynamic_switch(struct rockchip_pwm *pwm)
{
    return !pwm->is_int_enable && !pwm->is_counter_enable;
}

static void rockchip_pwm_irq_enable(struct rockchip_pwm *pwm, struct rt_pwm_configuration *config)
{
#if (PWM_MAIN_VERSION(PWM_VERSION_ID) >= 4)
...
static void rockchip_pwm_irq_disable(struct rockchip_pwm *pwm, struct rt_pwm_configuration *config)
{
#if (PWM_MAIN_VERSION(PWM_VERSION_ID) >= 4)
...
rt_err_t rockchip_pwm_control(struct rt_device_pwm *device, int cmd, void *arg)
{
    ...
    uint32_t freq_res_valid = 0x0;
    rt_err_t ret = RT_EOK;

    rk_pwm_dbg(device, "cmd: %d\n", cmd);
    rt_kprintf("cmd: %d\n", cmd);

    if (rockchip_pwm_support_pclk_dynamic_switch(pwm))
	...
	rk_pwm_dbg(device, " HAL_PWM_SetConfig ret: %d\n", ret);
        break;
    ...
    case PWM_CMD_INT_ENABLE:
        rockchip_pwm_irq_enable(pwm, config);
        pwm->is_int_enable = true;
        break;
    case PWM_CMD_INT_DISABLE:
        rockchip_pwm_irq_disable(pwm, config);
    ...
    case PWM_CMD_SET_FREQ:
        if (!pwm->freq_event)
            pwm->freq_event = rt_event_create("freq_event", RT_IPC_FLAG_FIFO);
        rockchip_pwm_irq_enable(pwm, config);
        clk_enable(pwm->clk_gate);
        HAL_PWM_EnableFreqMeter(pPWM, config->channel, config->delay);
        ret = rt_event_recv(pwm->freq_event, 0xffffffff, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,
                            config->delay * 3 / 2 * 1000, &freq_res_valid);
        if (ret || !freq_res_valid)
            break;
        ret = HAL_PWM_GetFreqMeterRes(pPWM, config->channel, &config->freqency);
