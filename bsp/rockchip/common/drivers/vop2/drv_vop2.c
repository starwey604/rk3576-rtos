...
static uint32_t drm_format_to_wfd_format(uint32_t format)
{
    int fmt;
    ...
    {
    case DRM_FORMAT_XRGB2101010:
        fmt = WFD_FORMAT_RGBX1010102;
        break;
    case DRM_FORMAT_ARGB2101010:
        fmt = WFD_FORMAT_RGBA1010102;
        break;
    case DRM_FORMAT_XBGR2101010:
        fmt = WFD_FORMAT_BGRX1010102;
        break;
    case DRM_FORMAT_ABGR2101010:
        fmt = WFD_FORMAT_BGRA1010102;
        break;
    case DRM_FORMAT_XRGB8888:
        fmt = WFD_FORMAT_RGBX8888;
        break;
    case DRM_FORMAT_ARGB8888:
        fmt = WFD_FORMAT_RGBA8888;
        break;
    case DRM_FORMAT_XBGR8888:
        fmt = WFD_FORMAT_BGRX8888;
        break;
    case DRM_FORMAT_ABGR8888:
        fmt = WFD_FORMAT_RGB888;
        break;
    case DRM_FORMAT_RGB888:
        fmt = WFD_FORMAT_RGB888;
        break;
    case DRM_FORMAT_RGB565:
        fmt = WFD_FORMAT_RGB565;
        break;
    case DRM_FORMAT_XRGB1555:
        fmt = WFD_FORMAT_RGBX5551;
        break;
    case DRM_FORMAT_ARGB1555:
        fmt = WFD_FORMAT_RGBA5551;
        break;
    case DRM_FORMAT_NV12:
        fmt = WFD_FORMAT_NV12;
        break;
...
void vop2_wfd_alloc_pipe(struct vop2 *vop2, struct wfd_pipe_data *pipe_data)
{
    ...
    for (i = 0; i < win->nformats; i++)
    {
        fmt = drm_format_to_wfd_format(win->formats[i]);
        if (fmt != WFD_FORMAT_INVALID)
        {
            pipe_data->formats[nformats] = fmt;
            nformats++;
        }
    }
    pipe_data->nformats = nformats;

    /*
     * The supported formats can be got by attribute WFD_PIPELINE_FORMATS_ROCKCHIP,
     * and it is needed to set at least one format in order to pass the validity
     * check.
     */
    if (!pipe_data->nformats)
    {
        pipe_data->nformats = 1;
        pipe_data->formats[0] = WFD_FORMAT_INVALID;
    }
}
