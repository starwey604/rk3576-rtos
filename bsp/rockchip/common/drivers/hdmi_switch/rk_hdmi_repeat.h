...
#define RK_RPT_LOG_ERROR      0
#define RK_RPT_LOG_INFO       1
#define RK_RPT_LOG_DEBUG      2

extern rt_uint8_t g_rk_rpt_log_level;

#define rk_rpt_err(fmt, ...) \
    do { \
        if(g_rk_rpt_log_level >= RK_RPT_LOG_ERROR) \
            rt_kprintf(fmt, ##__VA_ARGS__); \
    } while(0)

#define rk_rpt_info(fmt, ...) \
    do { \
        if(g_rk_rpt_log_level >= RK_RPT_LOG_INFO) \
            rt_kprintf(fmt, ##__VA_ARGS__); \
    } while(0)

#define rk_rpt_dbg(fmt, ...) \
    do { \
        if(g_rk_rpt_log_level >= RK_RPT_LOG_DEBUG) \
            rt_kprintf(fmt, ##__VA_ARGS__); \
    } while(0)
...
typedef enum
{
    RK_REPEAT_OUTPUT_INTERFACE_I2S,
    RK_REPEAT_OUTPUT_INTERFACE_SPDIF,
} rk_rpt_output_interface_type_t;
