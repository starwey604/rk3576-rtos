...
static rt_err_t _ep91a7p_write_regs(struct rt_i2c_client *client, void *write_buf, rt_uint8_t write_len)
{
    ...
    {
        rk_rpt_dbg("failed to write the register! %d\n", ret);
        return -RT_ERROR;
    }
}
...
static void ep91a7p_show_hdmi_decoderinfo(struct rk_rpt_audio_info *ado_info)
{
    if (ado_info == RT_NULL)
    {
        rk_rpt_err("ep91a7p_get_decoderinfo: ado_info is null!\n");
        return;
    }

    rk_rpt_info("++++++++++++++++++++++++++++++++++++++++++++++++++\n");

    rk_rpt_info("ep91a7p_get_decoderinfo:\n");

    rk_rpt_info("current mode:");
    ...
    if (!g_ep91a7p_dev->cur_arc_en)
    {
        ...
        rk_rpt_info("audio sample frequency is %d\n", ado_info->audio_sample_frequency);
        ...
        rk_rpt_info("audio sample word length is %d\n", ado_info->audio_sample_word_length);
    }
...
static rt_uint8_t ep91a7p_get_channel_frmo_ca(rt_uint8_t ca)
{
    ...
    case 0x31:
        ...
        break;
    default:
        rk_rpt_err("ep91a7p ca:%X force use 2ch\n", ca);
        channel_num = 2;
        break;
    }

    rk_rpt_dbg("ep91a7p ca:%X => %dch\n", ca, channel_num);
...
static void ep91a7p_get_ado_info(struct rk_rpt_audio_info *ado_info)
{
    ...
    ep91a7p_read_regs(EP_SYSTEM_STAT_0, 1, &g_ep91a7p_dev->ai.system_status_0);
    ep91a7p_read_regs(EP_AUDIO_STAT, 1, &g_ep91a7p_dev->ai.audio_status);
    ep91a7p_read_regs(EP_CHANNEL_STAT_0, 5, &g_ep91a7p_dev->ai.cs[0]);
    ep91a7p_read_regs(EP_ADO_INF_FRAME_1, 1, &g_ep91a7p_dev->ai.cc);
    ep91a7p_read_regs(EP_ADO_INF_FRAME_4, 1, &g_ep91a7p_dev->ai.ca);
    ...
    switch (ep_ado_inf_frame[2] & 0x03)
    {
    case 0:
    ...
    rk_rpt_dbg("sample word length is %d bits\n", ado_info->audio_sample_word_length);
    ...
    rk_rpt_dbg("final sample word length is %d bits\n", ado_info->audio_sample_word_length);

    if (ado_info->audio_sample_word_length == 0)
    {
        rk_rpt_dbg("sample word length force 16 bits\n");
        ado_info->audio_sample_word_length = 16;
    }
    ...
    {
        ado_info->CA = g_ep91a7p_dev->ai.ca;
    ...
    {
        ado_info->audio_output_inetrface = RK_REPEAT_OUTPUT_INTERFACE_SPDIF;
    }
            ...
            (g_ep91a7p_dev->cur_hdmi_en == 1 && g_ep91a7p_dev->hdmi_rx_plug_in == 1))
    {
        ...
        ado_info->audio_output_inetrface = RK_REPEAT_OUTPUT_INTERFACE_I2S;
    }
}
...
{
    rt_uint8_t is_changed = 0;

    if (ado_b->audio_output_inetrface == RK_REPEAT_OUTPUT_INTERFACE_SPDIF)
    {
        is_changed = (ado_a->audio_output_inetrface != ado_b->audio_output_inetrface);
    }
    else
    {
        is_changed = (ado_a->audio_channel_number != ado_b->audio_channel_number) ||
...
static rt_uint8_t ep91a7p_is_ado_info_valid(struct rk_rpt_audio_info *ado_info)
{
            ...
            ado_info->audio_output_inetrface == RK_REPEAT_OUTPUT_INTERFACE_I2S))
    {
        if ((ado_info->audio_sample_frequency != 0) &&
                (g_ep91a7p_dev->ai.system_status_0 & EP_AI_MCLK_ON_MASK))
        {
            return 1;
        }
    }
...
static void ep91a7p_get_general_control(ep91a7p_device_t *ep91a7p)
{
    ...
    ep91a7p_read_regs(EP_GENER_INF_0, 1, &ep91a7p->gi.tx_info);
    ep91a7p_read_regs(EP_GENERAL_CTL_1, 1, &g_ep91a7p_dev->gc.rx_sel);
    if (ep91a7p->gi.tx_info == 0xff)
    {
        rk_rpt_dbg("EP_GENER_INF_0 read 0xff\n");
    ...
    if (change)
    {
        rk_rpt_dbg("GI[0x08] old: 0x%02x, new: 0x%02x\n",
                   old, ep91a7p->gi.tx_info);
    }
    if (change & EP_GI_TX_HOT_PLUG_MASK)
    {
        val = (ep91a7p->gi.tx_info >> EP_GI_TX_HOT_PLUG_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("tx hotplug changed to %d\n", val);
    ...
    if (change & EP_GI_ARC_ON_MASK)
    {
        val = (ep91a7p->gi.tx_info >> EP_GI_EARC_ON_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("tx (e)arc_on changed to %d\n", arc_on_change);
        arc_on_change = 1;
    }
    ...
    {
        rk_rpt_dbg("EP_GENERAL_CTL_4 read 0xff\n");
        g_ep91a7p_dev->gc.link = old;
    }

    change = g_ep91a7p_dev->gc.link ^ old;
    if (change & EP_GC_LINK_MASK)
    {
        val = g_ep91a7p_dev->gc.link & EP_GC_LINK_MASK;
        rk_rpt_dbg("rx hotplug changed to 0x%x\n", val);
        g_ep91a7p_dev->hdmi_rx_plug_in = (val > 0) ? 1 : 0;
        hot_plug_change = 1;
    }
    ...
    old = g_ep91a7p_dev->ai.system_status_0;
    ep91a7p_read_regs(EP_SYSTEM_STAT_0, 1, &g_ep91a7p_dev->ai.system_status_0);
    if (ep91a7p->ai.system_status_0 == 0xff)
    {
        rk_rpt_dbg("EP_SYSTEM_STAT_0 read 0xff\n");
        g_ep91a7p_dev->ai.system_status_0 = old;
    }
    change = g_ep91a7p_dev->ai.system_status_0 ^ old;
    if (change & EP_AI_MCLK_ON_MASK)
    {
        val = (g_ep91a7p_dev->ai.system_status_0 >> EP_AI_MCLK_ON_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("i2s mclk on changed to %d\n", val);
        ado_change = 1;
    }

    old = ep91a7p->gi.video_latency;
    ep91a7p_read_regs(EP_GENER_INF_4, 1, &ep91a7p->gi.video_latency);
    if (ep91a7p->gi.video_latency == 0xff)
    {
        rk_rpt_dbg("EP_GENER_INF_4 read 0xff\n");
    ...
    if (change & EP_GI_VIDEO_LATENCY_MASK)
    {
        ...
        if (val > 0)
            val = (val - 1) * 2;
        rk_rpt_dbg("video latency changed to %d\n", val);
    }

    old = ep91a7p->gc.ctl;
    ep91a7p_read_regs(EP_GENERAL_CTL_0, 1, &ep91a7p->gc.ctl);
    if (ep91a7p->gc.ctl == 0xff)
    {
        rk_rpt_dbg("EP_GENERAL_CTL_0 read 0xff\n");
        ep91a7p->gc.ctl = old;
    }
    ...
    if (change & EP_GC_POWER_MASK)
    {
        val = (ep91a7p->gc.ctl >> EP_GC_POWER_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("power changed to %d\n", val);
    }
    if (change & EP_GC_AUDIO_PATH_MASK)
    {
        val = (ep91a7p->gc.ctl >> EP_GC_AUDIO_PATH_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("audio_path changed to %d\n", val);
    }
    if (change & EP_GC_CEC_MUTE_MASK)
    {
        val = (ep91a7p->gc.ctl >> EP_GC_CEC_MUTE_SHIFT) &
              EP_2CHOICE_MASK;
        rk_rpt_dbg("cec_mute changed to %d\n", val);
        ep91a7p_cecuserctl_mute();
    }

    if ((ep91a7p->gc.ctl & EP_GC_ARC_EN_MASK) || (ep91a7p->gc.ctl & EP_GC_EARC_EN_MASK))
    {
    ...
    {
        if (g_ep91a7p_dev->gc.rx_sel == 0x00 || g_ep91a7p_dev->gc.rx_sel == 0x01 ||
                g_ep91a7p_dev->gc.rx_sel == 0x02 || g_ep91a7p_dev->gc.rx_sel == 0x03)
        {
    ...
    ep91a7p_read_regs(EP_GENERAL_CTL_3, 1, &val);
...
static int ep91a7p_primary_sel(int primary_sel)
{
    ...
    {
    case 0:
        //port 0 select HDMI in
        reg = RT_EP91A7P_PRIMARY_SEL_PORT_0;
        ep91a7p_write_regs(EP_GENERAL_CTL_1, 1, &reg);
        break;
    case 1:
        //port 1 select HDMI in
        reg = RT_EP91A7P_PRIMARY_SEL_PORT_1;
        ep91a7p_write_regs(EP_GENERAL_CTL_1, 1, &reg);
        break;
    case 2:
        //port 2 select HDMI in
        reg = RT_EP91A7P_PRIMARY_SEL_PORT_2;
        ep91a7p_write_regs(EP_GENERAL_CTL_1, 1, &reg);
        break;
    case 3:
        //port 3 select HDMI in
        reg = RT_EP91A7P_PRIMARY_SEL_PORT_3;
        ep91a7p_write_regs(EP_GENERAL_CTL_1, 1, &reg);
        break;
    case 5:
        //port 5 select (e)ARC
        reg  = RT_EP91A7P_PRIMARY_SEL_PORT_5;
        ep91a7p_write_regs(EP_GENERAL_CTL_1, 1, &reg);
        break;
    default:
        rk_rpt_err("unsupported primary port select %d\n", primary_sel);
        break;
    }
...
static rt_err_t ep91a7p_ado_path_set(struct hdmi_repeat *rpt, void *data)
{
    ...
    case RK_REPEAT_AUDIO_PATH_HDMI_0:
        ...
        if (g_ep91a7p_dev->chip_id == EP_91A7P)
        {
            ep91a7p_primary_sel(1);
        }
        else if (g_ep91a7p_dev->chip_id == EP_92A7E)
        {
            reg &= ~(BIT(0) | BIT(6));
            ep91a7p_write_regs(EP_GENERAL_CTL_0, 1, &reg);
            ep91a7p_primary_sel(0);
        }
        if (g_ep91a7p_dev->last_ado_path == RK_REPEAT_AUDIO_PATH_UNKNOW &&
    ...
    case RK_REPEAT_AUDIO_PATH_HDMI_1:
        ...
        else if (g_ep91a7p_dev->chip_id == EP_92A7E)
        {
            ...
            ep91a7p_primary_sel(1);
            if (g_ep91a7p_dev->last_ado_path == RK_REPEAT_AUDIO_PATH_UNKNOW &&
                    g_ep91a7p_dev->last_vaild_ado_path == RK_REPEAT_AUDIO_PATH_HDMI_1)
            {
                g_ep91a7p_dev->last_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_1;
                ep91a7p_update_ado_info(1);
            }
            g_ep91a7p_dev->last_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_1;
            g_ep91a7p_dev->last_vaild_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_1;
    ...
    case RK_REPEAT_AUDIO_PATH_HDMI_2:
        ...
        else if (g_ep91a7p_dev->chip_id == EP_92A7E)
        {
            ...
            reg &= ~(BIT(0) | BIT(6));
            ep91a7p_write_regs(EP_GENERAL_CTL_0, 1, &reg);
            ep91a7p_primary_sel(2);
            if (g_ep91a7p_dev->last_ado_path == RK_REPEAT_AUDIO_PATH_UNKNOW &&
                    g_ep91a7p_dev->last_vaild_ado_path == RK_REPEAT_AUDIO_PATH_HDMI_2)
            {
                g_ep91a7p_dev->last_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_2;
                ep91a7p_update_ado_info(1);
            }
            g_ep91a7p_dev->last_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_2;
            g_ep91a7p_dev->last_vaild_ado_path = RK_REPEAT_AUDIO_PATH_HDMI_2;
...
static rt_err_t ep91a7p_ado_path_get(struct hdmi_repeat *rpt, void *data)
{
    ...
    if (g_ep91a7p_dev->chip_id == EP_91A7P)
    {
        if ((reg & EP_GC_ARC_EN_MASK) && (reg1 == RT_EP91A7P_PRIMARY_SEL_PORT_5))
        {
    ...
    {
        if ((reg & (EP_GC_ARC_EN_MASK | EP_GC_EARC_EN_MASK)) && (reg1 == RT_EP91A7P_PRIMARY_SEL_PORT_5))
        {
            *ado_path = RK_REPEAT_AUDIO_PATH_ARC_EARC;
        }
        else
        {
            switch (reg1 & EP_GC_RX_SEL_MASK)
            {
            case RT_EP91A7P_PRIMARY_SEL_PORT_0:
                *ado_path = RK_REPEAT_AUDIO_PATH_HDMI_0;
                break;
            case RT_EP91A7P_PRIMARY_SEL_PORT_1:
                *ado_path = RK_REPEAT_AUDIO_PATH_HDMI_1;
                break;
            case RT_EP91A7P_PRIMARY_SEL_PORT_2:
...
static void ep91a7p_dump()
{
    ...
    ep91a7p_read_regs(EP_GENERAL_CTL_1, 1, &g_ep91a7p_dev->gc.rx_sel);
    ep91a7p_read_regs(EP_GENERAL_CTL_2, 1, &g_ep91a7p_dev->gc.ctl2);
    ep91a7p_read_regs(EP_GENERAL_CTL_4, 1, &g_ep91a7p_dev->gc.link);
    ep91a7p_read_regs(EP_AUDIO_STAT, 1, &g_ep91a7p_dev->ai.audio_status);
    ...
    if (g_ep91a7p_dev->gc.ctl & EP_GC_POWER_MASK)
    {
        ...
        rt_kprintf("%x AudioSystemEnable[%d], ARC_enable[%d] eARC_enable[%d]\n", g_ep91a7p_dev->gc.ctl,
        ...
        rt_kprintf("Audio Ready, HDMI[%d], ARC[%d], eARC[%d]\n",
                   ((g_ep91a7p_dev->gc.link & EP_GC_LINK_MASK) != 0) ? 1 : 0,
...
static rt_uint8_t const ep_id[][5] =
{
    ...
    {EP_92A7E, EP92A7E_VENDOR_ID_0, EP92A7E_VENDOR_ID_1, EP92A7E_DEVICE_ID_0, EP92A7E_DEVICE_ID_1},
};
...
static rt_err_t ep91a7p_connectivity_check(void)
{
    ...
    //the i2c. The first i2c command may be lost, so we send it several
    //times to ensure successful awakening.
    for (trycount = 0; trycount < 60; trycount ++)
    {
        if (!ep91a7p_power_mode(EP91A7P_MODE_POWER_ON))
        {
            g_ep91a7p_dev->chip_id = ep91a7p_chip_match();
            if (g_ep91a7p_dev->chip_id == EP_DISMATCH)
            {
                return -RT_ERROR;
            }
            else
    ...
    rk_rpt_err("ep repeater not present!\n");
    return -RT_ERROR;
}
...
static rt_err_t ep91a7p_update_process(ISP_CTX *isp)
{
    ...
    rk_rpt_info("fw size %d,total %d block needs to be update\n", isp->fw_length, block_size);
    rt_thread_mdelay(10);
    ...
    if (ret)
    {
        rk_rpt_err("fw update failed\n");
    }
    else
    {
        rk_rpt_info("fw update done\n");
    }
...
static rt_err_t ep91a7p_update(struct rk_rpt_isp_op *op)
{
    ...
    if (!ret)
    {
                        ...
                        && (op->mode == RK_REPEAT_ISP_FORCE)))
        {
            ...
            if (!ret || g_ep91a7p_dev->chip_mode)
            {
                ...
                {
                    rk_rpt_err("FW isp i2c init failed\n");
                    ret = RK_REPEAT_ISP_UPDATE_FAILED;
                }
            }
            else
            {
                rk_rpt_err("FW enter isp mode failed\n");
                ret = RK_REPEAT_ISP_UPDATE_FAILED;
            }
        }
    }
    ...
    {
        rk_rpt_err("FW update init failed\n");
    }
...
static rt_err_t s_ep91a7p_init(ep91a7p_device_t *dev)
{
    ...
    dev->isr_sem = rt_sem_create("ep_isr_sem", 0, RT_IPC_FLAG_PRIO);
    RT_ASSERT(dev->isr_sem);

#ifdef EP91A7P_USE_RXMUTE_PIN
    dev->rx_mute_sem = rt_sem_create("ep_rxmute_sem", 0, RT_IPC_FLAG_PRIO);
...
static void ep91a7p_thread(void *arg)
{
    ...
    if (ep91a7p_connectivity_check())
    {
        ...
        {
            rk_rpt_err("ep repeater connectivity check failed\n");
            return;
        }
#else
        rk_rpt_err("ep repeater connectivity check failed\n");
...
#ifdef EP91A7P_USE_RESET_PIN
    //reset
    rt_pin_mode(EP91A7P_RST_PIN, PIN_MODE_OUTPUT);
    rt_pin_write(EP91A7P_RST_PIN, EP91A7P_RST_ON);
    rt_thread_mdelay(15);
    rt_pin_write(EP91A7P_RST_PIN, EP91A7P_RST_OFF);
#endif
...
static void ep91a7p_show_usage()
{
    ...
    rt_kprintf("ep91a7p audiopath <path>           -select path: 0:hdmi0 1:hdmi1 2:hdmi2 4:ARC/eARC mode\n");
...
#endif
    rt_kprintf("ep91a7p dbglevel <level>          -set debug level: 0.error,1.info,2.debug\n");
}
...
void ep91a7p(int argc, char **argv)
{
    ...
    else if (argc == 3)
    {
        ...
        {
            para = strtol(argv[2], RT_NULL, 10);
            rt_device_control(ep91a7p_dev, RK_REPEAT_CTL_DBG_LEVEL, (void *)&para);
        }
    }
    else if (argc == 4)
    {
