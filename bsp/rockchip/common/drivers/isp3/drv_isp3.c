...
#define HCLK_ISP3P2_GATE HCLK_ISP_GATE
#define CLK_CORE_ISP3P2_GATE CLK_CORE_ISP_GATE
#define PCLK_VICAP_GATE ISP0CLK_VICAP_GATE
#define PCLK_VICAP_VEPU_GATE NULL
#define I1CLK_VICAP_GATE NULL
#define I0CLK_VICAP_GATE NULL
#define RX1PCLK_VICAP_GATE NULL
#define RX0PCLK_VICAP_GATE NULL
#define PCLK_CSIHOST0_GATE PCLK_CSI2HOST0_GATE
#define PCLK_CSIHOST1_GATE PCLK_CSI2HOST1_GATE
#define PCLK_MIPICSIPHY0_GATE PCLK_CSIPHY0_GATE
#define PCLK_MIPICSIPHY1_GATE PCLK_CSIPHY1_GATE
#define CLK_RXBYTECLKHS_0_GATE DCLK_CSI2HOST0_GATE
#define CLK_RXBYTECLKHS_1_GATE DCLK_CSI2HOST1_GATE
#endif
                                       ...
                                       struct rk_camera_pix_format *format)
{
    ...
    ISP_INFO(dev, "et format:fourcc=0x%x,width=%d,height=%d\n",
             output_pix_format->pixelformat,
             output_pix_format->width,
             output_pix_format->height);
    if (output_pix_format->width < 1 || output_pix_format->height < 1)
    {
        ISP_INFO(dev, "Err: the width or height is invalid!\n");
        return -RT_EINVAL;
    }
                                   ...
                                   struct isp_video_requestbuffers *req)
{
    ...
    ISP_DBG(dev, "rk_isp_ctrl_reqbuf: width %d, height %d, num_buffers %d\n",
            width, height, num_buffers);
    if (width == 0 || height == 0)
    {
        ISP_INFO(dev, "Err: the width or height is 0!\n");
        return -RT_EINVAL;
    }
...
int rk_device_isp_init(void)
{
    ...
    ISP_INFO(rk_ispdev, "------ ISP device name: %s\n", rk_ispdev->name);
    ISP_INFO(rk_ispdev, "------ ISP share mem addr: 0x%x\n",
             isp_mem_config->share_mem_addr);
    ISP_INFO(rk_ispdev, "------ ISP share mem size: 0x%x\n",
             isp_mem_config->share_mem_size);
...
#elif defined SOC_RV1126B
    rk_ispdev->dphy_clk.dphy_pclk = get_clk_gate_from_id(PCLK_MIPICSIPHY0_GATE);
#endif
