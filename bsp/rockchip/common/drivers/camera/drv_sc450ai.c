...
#define SC450AI_REG_CHIP_ID_H           0x3107
#define SC450AI_REG_CHIP_ID_L           0x3108
...
static ret_err_t rk_sc450ai_set_expval(struct sc450ai_dev *dev, struct rk_camera_exp_val *exp)
{
    ...
    if (strcmp(dev->name, RK_ISP_ASSIST_SUBDEV_NAME) && !dev->has_init_exp && !dev->streaming)
    {
        dev->init_exp = *exp;
...
int rk_sc450ai_check_sensor_id(struct sc450ai_dev *dev)
{
    ret_err_t ret = RT_EOK;
    uint8_t id_h = 0, id_l = 0;

    ret = sc450ai_read_reg(dev->i2c_bus, SC450AI_REG_CHIP_ID_H, &id_h);
    ret |= sc450ai_read_reg(dev->i2c_bus, SC450AI_REG_CHIP_ID_L, &id_l);
    ...
    if (!ret)
    {
        ...
        if (id != CHIP_ID)
        {
            sc450ai_err("Unexpected sensor id(0x%02x), ret(%d)\n", id, ret);
            return RT_EINVAL;
        }
        sc450ai_info("Detected SC450AI(0x%04x) sensor\n", CHIP_ID);
    }

    sc450ai_info("check sensor id done, ret = %d\n", ret);
    return ret;
}
...
int rk_camera_sc450ai_init(void)
{
    ...
    instance->flip = 0;
    sc450ai_info("using sc450ai i2c_addr = 0x%02x\n", sc450ai->i2c_addr);
    ...
    if (sc450ai->rst_gpio.gpio_group)
    {
        ...
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc450ai->rst_gpio.gpio_group,
                             sc450ai->rst_gpio.gpio_pin,
                             sc450ai->rst_active);
        sc450ai_info("set reset gpio: %d, rst_active=%d \n", sc450ai->rst_gpio.gpio_pin, sc450ai->rst_active);
    }
    ...
    if (sc450ai->pwdn_gpio.gpio_group)
    {
        ...
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc450ai->pwdn_gpio.gpio_group,
                             sc450ai->pwdn_gpio.gpio_pin,
                             sc450ai->pwdn_active);
        sc450ai_info("set pwdn gpio: %d, pwdn_active=%d\n", sc450ai->pwdn_gpio.gpio_pin, sc450ai->pwdn_active);
    }
    ...
    if (sc450ai->mclk_out_gate_id && sc450ai->mclk_id)
    {
        ...
        sc450ai_info("mclk_id: 0x%x, mclk_out_gate_id=0x%x\n", sc450ai->mclk_id, sc450ai->mclk_out_gate_id);
    }
    ...
    rk_sc450ai_check_sensor_id(instance);
    return ret;
}
