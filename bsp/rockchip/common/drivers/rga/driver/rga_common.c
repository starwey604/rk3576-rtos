...
static void rga_dump_image_info(const char *name,
                                struct rga_img_info_t *img, uint8_t handle_flag, int need_mmu)
{
    if (handle_flag)
    {
        if (img->uv_addr && img->v_addr)
            pr_info("%s: handle[y,uv,v] = [%ld(%#lx), %ld(%#lx), %ld(%#lx)], mode = %s\n",
                    name,
            ...
            pr_info("%s: handle[y,uv] = [%ld(%#lx), %ld(%#lx)], mode = %s\n",
                    name,
                    (unsigned long)img->yrgb_addr, (unsigned long)img->yrgb_addr,
                    (unsigned long)img->uv_addr, (unsigned long)img->uv_addr,
                    rga_get_store_mode_str(img->rd_mode));
        else
            pr_info("%s: handle = %ld(%#lx), mode = %s\n",
                    name,
                    (unsigned long)img->yrgb_addr, (unsigned long)img->yrgb_addr,
                    rga_get_store_mode_str(img->rd_mode));
    }
    ...
    {
        if (img->yrgb_addr)
            pr_info("%s: fd = %ld(%#lx), mode = %s\n",
                    name,
                    (unsigned long)img->yrgb_addr, (unsigned long)img->yrgb_addr,
                    rga_get_store_mode_str(img->rd_mode));
        else if (img->uv_addr)
            pr_info("%s: %s = %#lx, mode = %s\n",
                    name,
                    need_mmu ? "virt_addr" : "phys_addr", (unsigned long)img->uv_addr,
                    rga_get_store_mode_str(img->rd_mode));
    }

    pr_info("%s: rect[x,y,w,h] = [%d, %d, %d, %d], stride[w,h] = [%d, %d], format = %s(%#x)\n",
            name,
...
void rga_dump_req(struct rga_req *req)
{
    ...
    rga_dump_image_info("src", &req->src, req->handle_flag,
                        (req->mmu_info.mmu_flag >> 8) & 1);
    if ((req->pat.yrgb_addr != 0) || (req->pat.uv_addr != 0) || (req->pat.v_addr != 0))
        rga_dump_image_info("pat", &req->pat, req->handle_flag,
                            (req->mmu_info.mmu_flag >> 9) & 1);
    rga_dump_image_info("dst", &req->dst, req->handle_flag,
