...
static unsigned int rga2_rop_code[256] =
{
...
static void RGA2_set_reg_src_info(u8 *base, struct rga2_req *msg)
{
    ...
    u32 head_base_addr = 0, payload_base_addr = 0;
...
static void RGA2_set_mmu_reg_info(struct rga_scheduler *scheduler, u8 *base, struct rga2_req *msg)
{
    u32 *bRGA_MMU_CTRL1;
...
static int rga2_gen_reg_info(struct rga_scheduler *scheduler, u8 *base, struct rga2_req *msg)
{
    int ret;
    u8 dst_nn_quantize_en = 0;
...
static void rga_cmd_to_rga2_cmd(struct rga_scheduler *scheduler,
...
static void rga2_soft_reset(struct rga_scheduler *scheduler)
{
    u32 i;
    u32 reg;
...
static int rga2_init_reg(struct rga_job *job)
{
    ...
    if (scheduler->data->mmu == RGA_IOMMU || RGA_DIS_MASETR_MODE)
...
static void rga2_dump_read_back_sys_reg(struct rga_scheduler *scheduler)
{
    int i;
    unsigned long flags;
...
static void rga2_dump_read_back_csc_reg(struct rga_scheduler *scheduler)
{
    int i;
    unsigned long flags;
...
static void rga2_dump_read_back_cmd_reg(struct rga_scheduler *scheduler)
{
    int i;
    unsigned long flags;
...
static void rga2_dump_read_back_reg(struct rga_scheduler *scheduler)
{
...
static void rga2_set_pre_intr_reg(struct rga_job *job, struct rga_scheduler *scheduler)
{
    uint32_t reg;
...
static void rga2_set_reg_full_csc(struct rga_job *job, struct rga_scheduler *scheduler)
{
...
static int rga2_set_reg(struct rga_job *job, struct rga_scheduler *scheduler)
{
    int i;
    bool master_mode_en;
    uint32_t sys_ctrl;
    ...
    if (master_mode_en)
    {
        ...
        /* set cmd_addr */
        rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)job->cmd_buf->dma_addr, job->cmd_buf->size);
...
static int rga2_get_version(struct rga_scheduler *scheduler)
{
    ...
    snprintf((char *)scheduler->version.str, RGA_VERSION_SIZE - 1, "%02x.%01x.%05x",
...
static int rga2_read_back_reg(struct rga_job *job, struct rga_scheduler *scheduler)
{
...
static int rga2_read_status(struct rga_job *job, struct rga_scheduler *scheduler)
{
...
static int rga2_irq(struct rga_scheduler *scheduler)
{
...
static int rga2_isr_thread(struct rga_job *job, struct rga_scheduler *scheduler)
{
    if (DEBUGGER_EN(INT_FLAG))
