...
static inline int rga_read(rt_off_t offset, struct rga_scheduler *scheduler)
{
    return readl((uintptr_t)(scheduler->rga_base + offset));
}

static inline void rga_write(int value, rt_off_t offset, struct rga_scheduler *scheduler)
{
    writel(value, (uintptr_t)(scheduler->rga_base + offset));
}
