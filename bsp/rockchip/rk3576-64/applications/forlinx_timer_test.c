...
static void forlinx_timer_test(int argc, char **argv)
{
    ...
    if(argc > 1)
    {
        ...
        {
            nest = 1;
            rt_hw_interrupt_install(TIMER6_IRQn, TIMER6_tick_isr, RT_NULL, RT_NULL);
            rt_hw_interrupt_umask(TIMER6_IRQn);
            HAL_TIMER_Init(TIMER6, TIMER_FREE_RUNNING);
            HAL_TIMER_SetCount(TIMER6, (PLL_INPUT_OSC_RATE / 10) - 1);
            printf("set TIMER6 100ms\n");

            rt_hw_interrupt_install(TIMER7_IRQn, TIMER7_tick_isr, RT_NULL, RT_NULL);
            rt_hw_interrupt_umask(TIMER7_IRQn);
            HAL_TIMER_Init(TIMER7, TIMER_FREE_RUNNING);
            HAL_TIMER_SetCount(TIMER7, (PLL_INPUT_OSC_RATE / 100) - 1);
            printf("set TIMER7 10ms\n");

            HAL_TIMER_Start_IT(TIMER6);
            HAL_TIMER_Start_IT(TIMER7);
            count = 0;
            while(1)
            {
                if(count >= 5)
            ...
            HAL_TIMER_Stop_IT(TIMER6);
            printf("TIMER6 over\n");
            HAL_TIMER_Stop_IT(TIMER7);
            printf("TIMER7 over\n");

        }else{
            rt_kprintf("Usage: \n");
            rt_kprintf("forlinx_timer time <dev_name>   - Test TIMER timed interruption\n");
            rt_kprintf("forlinx_timer nest              - Test TIMER6 and TIMER7 interrupt nesting\n");
            rt_kprintf("\t<dev_name> : 6 ~ 10 \n");
        }
    }else{
        rt_kprintf("Usage: \n");
