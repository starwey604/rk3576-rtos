...
void drm_dp_link_train_clock_recovery_delay(const u8 dpcd[DP_RECEIVER_CAP_SIZE])
{
	...
	/*
	 * According to the description in DPCD reg TRAINING_AUX_RD_INTERVAL(0x0000Eh).
	 */
	if (rd_interval > 4)
...
u8 drm_dp_link_rate_to_bw_code(int link_rate)
{
	...
	default:
		printf("unknown DP link rate %d, using %x\n", link_rate, DP_LINK_BW_1_62);
		return DP_LINK_BW_1_62;
	}
}
...
int drm_dp_bw_code_to_link_rate(u8 link_bw)
{
	...
	case DP_LINK_BW_8_1:
		return 810000;
	default:
		printf("unknown DP link BW code %x, using 162000\n", link_bw);
		return 162000;
	}
}
...
int drm_dp_dpcd_read(struct drm_dp_aux *aux, unsigned int offset,
...
int drm_dp_dpcd_write(struct drm_dp_aux *aux, unsigned int offset,
		      void *buffer, unsigned int size)
{
	int ret;
