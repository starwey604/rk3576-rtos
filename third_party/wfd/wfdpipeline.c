		      ...
		      WFDPipelineConfigAttrib attr) WFD_APIEXIT
{
	...
	case WFD_PIPELINE_GLOBAL_ALPHA:
		...
		break;
	case WFD_PIPELINE_FORMAT_COUNT_ROCKCHIP:
		value = pipe->nformats;
		break;
	default:
		wfderr = WFD_ERROR_BAD_ATTRIBUTE;
		       ...
		       WFDint *value) WFD_APIEXIT
{
	...
	case WFD_PIPELINE_DESTINATION_RECTANGLE:
		...
		break;
	case WFD_PIPELINE_FORMATS_ROCKCHIP:
#if !WFD_STRICT
		while(count > pipe->nformats) {
			value[--count] = WFD_FORMAT_INVALID;
		}
#endif
		if (count == pipe->nformats) {
			while (count > 0) {
				count--;
				value[count] = pipe->formats[count];
			}
		} else {
			wfderr = WFD_ERROR_ILLEGAL_ARGUMENT;
		}
